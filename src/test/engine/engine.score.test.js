...
describe("getScoreObject tests", () => {
  let testScoreObj = getScoreObject(
    createHand([14, 12, 13, 11, 10], ["s", "s", "s", "s", "s"])
  );

  test("should throw error if not provided an array of correct size", () => {
    expect(() =>
      getScoreObject(
        createHand([2, 3, 4, 5, 6, 7, 8], ["c", "c", "c", "c", "c", "c", "c"])
      )
    ).toThrow();
  });
  ...
  test("score object should contain array of cards used", () => {
    expect(testScoreObj.cardsUsed).toEqual(
      expect.arrayContaining([new Card(10, "Spades")])
    );
    expect(testScoreObj.cardsUsed.length).toEqual(5);
  });

  test("should properly analyze a straight flush", () => {
    let sFTestScoreObj = getScoreObject(
      createHand([14, 12, 13, 11, 10], ["s", "s", "s", "s", "s"])
    );
    expect(sFTestScoreObj.score).toEqual(8);
    expect(sFTestScoreObj.highHandCards).toEqual(
      expect.arrayContaining([new Card(14, "Spades")])
    );
    ...
    sFTestScoreObj = getScoreObject(
      createHand([3, 4, 5, 6, 7], ["h", "h", "h", "h", "h"])
    );
    expect(sFTestScoreObj.score).toEqual(8);
    // ensure low ace is not counted as the high card in a straight
    sFTestScoreObj = getScoreObject(
      createHand([2, 3, 4, 5, 14], ["h", "h", "h", "h", "h"])
    );
    expect(sFTestScoreObj.highHandCards).toEqual(
      expect.arrayContaining([new Card(5, "Hearts")])
    );
    expect(sFTestScoreObj.highHandCards).toHaveLength(1);
  ...
  test("should recognize an ace has value 1 or 14", () => {
    let aceTestScoreObj = getScoreObject(
      createHand([14, 2, 3, 4, 5], ["s", "s", "s", "s", "s"])
    );
    expect(aceTestScoreObj.score).toEqual(8);
    aceTestScoreObj = getScoreObject(
      createHand([14, 12, 13, 11, 10], ["s", "s", "s", "s", "s"])
    );
    expect(aceTestScoreObj.score).toEqual(8);
  });
  ...
  test("should properly analyze four of a kind", () => {
    const fourTestScoreObj = getScoreObject(
      createHand([4, 4, 4, 4, 1], ["h", "d", "c", "s", "h"])
    );
    expect(fourTestScoreObj.score).toEqual(7);
    expect(fourTestScoreObj.highHandCards).toEqual(
      expect.arrayContaining([new Card(4, "Hearts")])
    );
    expect(fourTestScoreObj.highHandCards).toHaveLength(1);
    expect(fourTestScoreObj.type).toEqual("four of a kind");
    expect(fourTestScoreObj.cardsUsed).not.toEqual(
      expect.arrayContaining([new Card(1, "Hearts")])
    ); // the unused card should not be in cardsUsed
  });

  test("should properly analyze a full house", () => {
    let fullTestScoreObj = getScoreObject(
      createHand([12, 13, 12, 13, 12], ["s", "c", "d", "h", "s"])
    );
    ...
    fullTestScoreObj = getScoreObject(
      createHand([3, 3, 3, 2, 2], ["s", "c", "d", "h", "s"])
    );
    expect(fullTestScoreObj.score).toEqual(6);
  });

  test("should properly analyze a straight", () => {
    let straightTestScoreObj = getScoreObject(
      createHand([9, 8, 7, 6, 5], ["s", "c", "d", "h", "s"])
    );
    expect(straightTestScoreObj.score).toEqual(5);
    expect(straightTestScoreObj.highHandCards).toEqual(
      expect.arrayContaining([new Card(9, "Spades")])
    );
    ...
    straightTestScoreObj = getScoreObject(
      createHand([2, 3, 4, 5, 14], ["s", "c", "d", "h", "s"])
    );
    expect(straightTestScoreObj.score).toEqual(5);
    // TODO: debug this test:
    straightTestScoreObj = getScoreObject(
      createHand([2, 3, 4, 14, 14], ["h", "c", "d", "h", "c"])
    );
    expect(straightTestScoreObj.score).not.toEqual(5);
  });

  test("should properly analyze a flush", () => {
    let flushTestScoreObj = getScoreObject(
      createHand([9, 10, 7, 4, 5], ["c", "c", "c", "c", "c"])
    );
    expect(flushTestScoreObj.score).toEqual(4);
    expect(flushTestScoreObj.highHandCards).toEqual(
      expect.arrayContaining([new Card(9, "Clubs")])
    );
    ...
    flushTestScoreObj = getScoreObject(
      createHand([11, 12, 13, 14, 4], ["h", "h", "h", "h", "h"])
    );
    expect(flushTestScoreObj.score).toEqual(4);
  });

  test("should properly analyze three of a kind", () => {
    let threeTestScoreObj = getScoreObject(
      createHand([9, 9, 9, 4, 5], ["c", "h", "s", "c", "c"])
    );
    expect(threeTestScoreObj.score).toEqual(3);
    expect(threeTestScoreObj.highHandCards).toEqual(
      expect.arrayContaining([new Card(9, "Clubs")])
    );
    ...
    expect(threeTestScoreObj.cardsUsed).not.toEqual(
      expect.arrayContaining([new Card(4, "Clubs"), new Card(5, "Clubs")])
    ); // the unused cards should not be in cardsUsed
    threeTestScoreObj = getScoreObject(
      createHand([4, 2, 12, 12, 12], ["h", "c", "s", "d", "c"])
    );
    expect(threeTestScoreObj.score).toEqual(3);
  });

  test("should properly analyze two pair", () => {
    let twoPairTestScoreObj = getScoreObject(
      createHand([8, 8, 4, 4, 5], ["c", "h", "s", "c", "c"])
    );
    ...
    expect(twoPairTestScoreObj.cardsUsed).not.toEqual(
      expect.arrayContaining([new Card(5, "Clubs")])
    ); // the unused card should not be in cardsUsed
    twoPairTestScoreObj = getScoreObject(
      createHand([3, 2, 11, 11, 3], ["h", "c", "s", "d", "c"])
    );
    expect(twoPairTestScoreObj.score).toEqual(2);
  });

  test("should properly analyze a pair", () => {
    let pairTestScoreObj = getScoreObject(
      createHand([6, 6, 3, 4, 5], ["c", "h", "s", "c", "c"])
    );
    ...
    expect(pairTestScoreObj.cardsUsed).not.toEqual(
    ...
    pairTestScoreObj = getScoreObject(
      createHand([11, 14, 2, 7, 14], ["h", "c", "s", "d", "c"])
    );
    expect(pairTestScoreObj.score).toEqual(1);
  });

  test("should return a 0-score object on a hand with high card only", () => {
    let highCardTestScoreObj = getScoreObject(
      createHand([6, 9, 3, 4, 5], ["c", "h", "s", "c", "c"])
    );
    ...
    highCardTestScoreObj = getScoreObject(
      createHand([11, 14, 3, 7, 12], ["h", "c", "s", "d", "c"])
    );
    expect(highCardTestScoreObj.score).toEqual(0);
  });
});
...
describe("getScore basic tests: ", () => {
  let testScoreObj = getScore(
    createHand([11, 2], ["h", "d"]),
    createHand([3, 4, 5, 6, 9], ["d", "d", "s", "c", "d"]),
    "test"
  );
...
describe("comprehensive scoring tests for getScore:", () => {
  let testScoreObj = getScore(
    createHand([4, 4], ["s", "s"]),
    createHand([4, 4, 3, 3, 3], ["s", "c", "s", "c", "d"]),
    "test"
  );
  ...
  test("should choose the best 2 of 3 pairs", () => {
    testScoreObj = getScore(
      createHand([12, 12], ["s", "d"]),
      createHand([2, 11, 2, 7, 7], ["s", "c", "h", "c", "d"]),
      "test"
    );
  ...
  test("should choose the highest straight", () => {
    testScoreObj = getScore(
      createHand([2, 3], ["s", "d"]),
      createHand([4, 5, 6, 7, 8], ["s", "c", "h", "c", "d"]),
      "test"
    );
    expect(testScoreObj.score).toEqual(5);
  ...
  test("should choose a straight over a flush", () => {
    testScoreObj = getScore(
      createHand([14, 3], ["s", "s"]),
      createHand([4, 5, 6, 7, 9], ["s", "d", "s", "s", "s"]),
      "test"
    );
    expect(testScoreObj.score).toEqual(5);
    expect(testScoreObj.highHandCards).toHaveLength(1);
  ...
  test("should choose a flush over 3 of a kind", () => {
    testScoreObj = getScore(
      createHand([11, 3], ["s", "d"]),
      createHand([3, 3, 12, 7, 4], ["s", "h", "s", "s", "s"]),
      "test"
    );
  ...
  test("should choose a straight flush over a higher-number straight", () => {
    testScoreObj = getScore(
      createHand([13, 8], ["s", "d"]),
      createHand([13, 12, 11, 10, 9], ["h", "d", "d", "d", "d"]),
      "test"
    );
